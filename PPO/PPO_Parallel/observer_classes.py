...
class PrintStatusObserver:
  ...
  def __init__(
      self, status_interval_steps: int = 1, environment=None, replay_buffer=None, num_timesteps_in_episode=None
  ):
    self._counter = 0
    ...
    self._start_time = None
    if num_timesteps_in_episode is not None:
      self._num_timesteps_in_episode = num_timesteps_in_episode
  ...
  def __call__(self, trajectory: trajectory_lib.Trajectory) -> None:
    reward = np.mean(trajectory.reward) # in case of parallel env
    self._cumulative_reward += reward
    self._counter += 1
    if self._start_time is None:
      self._start_time = pd.Timestamp.now()
    ...
    if self._counter % self._status_interval_steps == 0 and self._environment:
      ...
      percent_complete = int(
          100.0 * (self._counter % self._num_timesteps_in_episode  / self._num_timesteps_in_episode)
      )
      ...
      print(
          ...
          % (
              self._counter % self._num_timesteps_in_episode,
              self._num_timesteps_in_episode,
              percent_complete,
    ...
    # # print(f"Completed Step Percentage: {self._counter / self._num_timesteps_in_episode * 100:.2f}%")
